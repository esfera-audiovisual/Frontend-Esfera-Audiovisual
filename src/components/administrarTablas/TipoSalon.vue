...
import { useStoreTipoSalon } from '../../stores/tipo.js'; // Asegúrate de tener la store correspondiente
...
const columns = [
    { name: "nombre_tip", label: "Nombre del tipo de salón", field: "nombre_tip", sortable: true, align: "left" },
...
const enviarInfo = {
    agregar: async () => {
        try {
            ...
            const response = await useTipoSalon.registro(data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
            ...
            modal.value = false;
            notificar('positive', 'Tipo de salón agregado exitosamente');
        } catch (error) {
            console.log(error);
        } finally {
    ...
    editar: async () => {
        loadingModal.value = true;
        try {
            const response = await useTipoSalon.editar(data.value._id, data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
            ...
            modal.value = false;
            notificar('positive', 'Tipo de salón editado exitosamente');
        } catch (error) {
            console.log(error);
        } finally {
            loadingModal.value = false;
...
const in_activar = {
    activar: async (id) => {
        ...
        try {
            const response = await useTipoSalon.activar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
    ...
    inactivar: async (id) => {
        loadIn_activar.value = true;
        try {
            const response = await useTipoSalon.inactivar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
...
<template>
    <main style="width: 100%; display: flex; justify-content: center;">
        ...
        <q-dialog v-model="modal">
            <q-card class="modal" style="width: 450px;">
                <q-toolbar class="bg-primary" style="color: white">
                    <q-toolbar-title>{{ estado === 'agregar' ? 'Agregar' : 'Editar' }} Tipo de Salón</q-toolbar-title>
                    <q-btn class="botonv1" flat dense icon="close" v-close-popup />
                </q-toolbar>
                <q-card-section class="q-gutter-md">
                    <q-form @submit="validarCampos" class="q-gutter-md">
                        <q-input filled v-model.trim="data.nombre_tip" label="Nombre del tipo de salón"
        ...
        <q-table :rows="rows" :columns="columns" row-key="nombre_tip" :loading="loadingTable"
            ...
            no-results-label="No hay resultados para la búsqueda." wrap-cells="false" label="Tipos de Salón"
            style="width: 90%; margin-top: 40px;" no-data-label="No hay tipos de salón registrados.">

            <template v-slot:top-left>
                <div style="display: flex; gap: 10px;">
                    <h4 id="titleTable">Tipos de Salón</h4>
