...
import { useStoreServicioSalon } from '../../stores/servicio.js'; // Asegúrate de tener la store correspondiente
...
const enviarInfo = {
    agregar: async () => {
        try {
            ...
            const response = await useServicio.registro(data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
    ...
    editar: async () => {
        loadingModal.value = true;
        try {
            const response = await useServicio.editar(data.value._id, data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
            ...
            <q-card class="modal" style="width: 450px;">
                ...
                <q-card-section class="q-gutter-md">
                    <q-form @submit="validarCampos" class="q-gutter-md">
                        <q-input filled v-model.trim="data.nombre_serv" label="Nombre del servicio"
