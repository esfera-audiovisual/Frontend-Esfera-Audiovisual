...
import { useStoreEspacioSalon } from '../../stores/espacio.js'; // Asegúrate de tener la store correspondiente
...
const columns = [
    { name: "nombre_esp", label: "Nombre del espacio", field: "nombre_esp", sortable: true, align: "left" },
...
const enviarInfo = {
    agregar: async () => {
        try {
            ...
            const response = await useEspacio.registro(data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
            ...
            modal.value = false;
            notificar('positive', 'Espacio agregado exitosamente');
        } catch (error) {
            console.log(error);
        } finally {
    ...
    editar: async () => {
        loadingModal.value = true;
        try {
            const response = await useEspacio.editar(data.value._id, data.value);
            getInfo();
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
            ...
            modal.value = false;
            notificar('positive', 'Espacio editado exitosamente');
        } catch (error) {
            console.log(error);
        } finally {
            loadingModal.value = false;
...
const in_activar = {
    activar: async (id) => {
        ...
        try {
            const response = await useEspacio.activar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
    ...
    inactivar: async (id) => {
        loadIn_activar.value = true;
        try {
            const response = await useEspacio.inactivar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
...
<template>
    <main style="width: 100%; display: flex; justify-content: center;">
        ...
        <q-dialog v-model="modal">
            <q-card class="modal" style="width: 450px;">
                <q-toolbar class="bg-primary" style="color: white">
                    <q-toolbar-title>{{ estado === 'agregar' ? 'Agregar' : 'Editar' }} Espacio</q-toolbar-title>
                    <q-btn class="botonv1" flat dense icon="close" v-close-popup />
                </q-toolbar>
                <q-card-section class="q-gutter-md">
                    <q-form @submit="validarCampos" class="q-gutter-md">
                        <q-input filled v-model.trim="data.nombre_esp" label="Nombre del espacio"
        ...
        <!-- Tabla -->
        <q-table :rows="rows" :columns="columns" row-key="nombre_esp" :loading="loadingTable"
            loading-label="Cargando..." :filter="filter" rows-per-page-label="Visualización de filas"
            no-results-label="No hay resultados para la búsqueda." wrap-cells="false" label="Espacios"
            style="width: 90%; margin-top: 40px;" no-data-label="No hay espacios registrados.">

            <template v-slot:top-left>
                <div style="display: flex; gap: 10px;">
                    <h4 id="titleTable">Espacios</h4>
