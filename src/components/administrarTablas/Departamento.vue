...
import { useStoreDepartamentoSalonEvento } from '../../stores/departamento.js'; // Asegúrate de tener la store correspondiente
import { useRouter } from 'vue-router';

const useDepartamento = useStoreDepartamentoSalonEvento();
...
const columns = [
    { name: "nombre_depart", label: "Nombre del departamento", field: "nombre_depart", sortable: true, align: "left" },
...
const in_activar = {
    activar: async (id) => {
        ...
        try {
            const response = await useDepartamento.activar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
    ...
    inactivar: async (id) => {
        loadIn_activar.value = true;
        try {
            const response = await useDepartamento.inactivar(id);
            if (!response) return;
            if (response.error) {
                notificar('negative', response.error);
                return;
...
<template>
    <main style="width: 100%; display: flex; justify-content: center;">
        ...
        <q-dialog v-model="modal">
            <q-card class="modal" style="width: 450px;">
                <q-toolbar class="bg-primary" style="color: white">
                    <q-toolbar-title>{{ estado === 'agregar' ? 'Agregar' : 'Editar' }} Departamento</q-toolbar-title>
                    <q-btn class="botonv1" flat dense icon="close" v-close-popup />
                </q-toolbar>

                <q-card-section class="q-gutter-md">
                    <q-form @submit="validarCampos" class="q-gutter-md">
                        <q-input filled v-model.trim="data.nombre_depart" label="Nombre del departamento"
                            :rules="[val => !!val || 'Digite el nombre']" />

                        <q-input filled v-model.trim="data.latitud" label="Latitud"
                            :rules="[val => !!val || 'Digite la latitud']" />

                        <q-input filled v-model.trim="data.longitud" label="Longitud"
        ...
        <!-- Tabla -->
        <q-table :rows="rows" :columns="columns" row-key="nombre_depart" :loading="loadingTable"
            ...
            wrap-cells="false" label="Departamentos" style="width: 90%; margin-top: 40px;"
            no-data-label="No hay departamentos registrados.">

            <template v-slot:top-left>
                <div style="display: flex; gap: 10px;">
                    <h4 id="titleTable">Departamentos</h4>
            ...
            <template v-slot:body-cell-opciones="props">
                <q-td :props="props">
                    <div id="buttons">
                        ...
                        <button class="editBtn bg-primary" @click="verCiudades(props.row)">
                            Ver ciudades
                        </button>
                    </div>
                </q-td>
